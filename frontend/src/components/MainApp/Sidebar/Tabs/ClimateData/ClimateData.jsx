...
import { default as SensorData } from "./SensorData/SensorData";
...
function ClimateData() {
  ...
  const [checkedItems, setCheckedItems] = useState(
    climateDataContext.climateDataOn
  );
  ...
  const [climateTabOpen, setClimateTabOpen] = useState(
    ...
    <>
      <div style={{ display: "flex", flexDirection: "column" }}>
        {climateTabOpen !== true ? (
          ...
          <div className={styles.sectionContent}>
            <h4>{dataOption.name}</h4>
            <div className={styles.bodyContent}>{dataOption.component}</div>
          </div>
        )}
      </div>
    </>
